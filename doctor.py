...
def show_appointments():
    doctor_cursor.execute("SELECT * FROM Appointments")
    print("ID\tPatient ID\tTime\tDr ID\tStatus")
    print("--------------------------------------------------")
    for appointment in doctor_cursor.fetchall():
...
def cancel_appointment():
    ...
    doctor_cursor.execute("DELETE FROM Appointments WHERE Appointments.ID = %s", int(appointment_id))
    doctor_db.commit()
    # send email to the patient


def show_drug_usage_history():
    print("Enter the patient's ID:")
    patient_id = input()
    doctor_cursor.execute("SELECT DISTINCT Drugs.Name FROM Prescription, Drugs WHERE Prescription.DrugID = Drugs.ID AND"
                          " Prescription.AppointmentID = (SELECT ID FROM Appointments WHERE PatientID = %s)",
                          patient_id)
    for drug in doctor_cursor.fetchall():
        print(drug[0])
